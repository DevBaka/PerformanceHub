...
public sealed class MainViewModel : ObservableObject
{
    ...
    // Guards against overlapping display-topology calls (e.g. impatient double/triple-clicking while a
    // switch is still settling) - stacking another change on top of one the driver hasn't finished
    // processing yet has been observed to hang the display engine hard enough to force a full reboot.
    ...
    public MainViewModel(ILogSink log)
    {
        ...
        Log.EntryLogged += entry => Dispatcher.UIThread.Post(() => AppendLogLine(entry));
