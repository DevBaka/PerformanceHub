...
internal sealed class WindowsServiceControlBackend : IServiceControlBackend
{
    ...
    public IReadOnlyList<(string Name, string DisplayName, ServiceRunState Status, string? StartupType)> ListAll()
    {
        ...
        {
            try { result.Add((sc.ServiceName, sc.DisplayName, Convert(sc.Status), GetStartupType(sc.ServiceName))); }
            catch { /* skip services we can't query */ }
        }
        return result;
    }

    private static ServiceRunState Convert(ServiceControllerStatus status) => status switch
    {
