...
public partial class DisplayModeDialog : Window
{
    ...
    public DisplayModeDialog(DisplayManager displayManager, DisplayProfileEntry entry)
    {
        ...
        HintText.Text = _allModes.Count == 0
            ...
            : dpiHint + " Hinweis: Windows zeigt bei manchen Bildwiederholraten Kommawerte (z.B. 119.88 statt 120 Hz) - diese Liste rundet auf ganze Zahlen. " +
              "Für exakte Kommawerte: einmal in Windows selbst einstellen, dann hier 'Aktuelle Anordnung laden' nutzen.";
    ...
    private void BuildRefreshOptions(int width, int height)
    {
        ...
        if (!rates.Contains(previous)) RefreshCombo.Text = rates.FirstOrDefault() ?? previous;
        else RefreshCombo.Text = previous;
        _suppressEvents = false;
    }
    ...
    private void Ok_Click(object sender, RoutedEventArgs e)
    {
        if (!string.IsNullOrWhiteSpace(ResolutionCombo.Text))
        {
            if (!TryParseResolution(ResolutionCombo.Text, out var w, out var h))
            {
                MessageBox.Show("Auflösung bitte im Format BREITExHÖHE angeben, z.B. 1920x1080.", "ProfileDeck", MessageBoxButton.OK, MessageBoxImage.Warning);
                return;
            }
            if (!ConfirmIfCustom("Auflösung", ResolutionCombo.Text, ResolutionCombo.Items)) return;
            ResultWidth = w;
            ResultHeight = h;
        }
        ...
        if (!string.IsNullOrWhiteSpace(RefreshCombo.Text))
        {
            ...
            {
                MessageBox.Show("Bildwiederholrate bitte als Zahl angeben, z.B. 60 oder 144.", "ProfileDeck", MessageBoxButton.OK, MessageBoxImage.Warning);
                return;
            }
            if (!ConfirmIfCustom("Bildwiederholrate", RefreshCombo.Text, RefreshCombo.Items)) return;
            ResultRefreshHz = hz;
        }
        ...
        if (!string.IsNullOrWhiteSpace(ScalingCombo.Text))
        {
            ...
            {
                MessageBox.Show("Skalierung bitte als Prozentzahl angeben, z.B. 150.", "ProfileDeck", MessageBoxButton.OK, MessageBoxImage.Warning);
                return;
            }
            if (!ConfirmIfCustom("Skalierung", ScalingCombo.Text, ScalingCombo.Items)) return;
    ...
    /// <summary>
    /// If the typed text doesn't match one of the dropdown's known-good values exactly, this is a value
    /// Windows/the driver never advertised as supported - warn and require an explicit OK before proceeding,
    /// since applying it can silently fall back to a completely different mode (e.g. the monitor's default)
    /// with no further feedback once the profile is actually applied.
    /// </summary>
    private bool ConfirmIfCustom(string fieldLabel, string enteredText, System.Collections.IEnumerable knownItems)
    {
        ...
        var result = MessageBox.Show(
            ...
            "Windows/der Treiber könnte diesen Wert ablehnen - dann wird beim Anwenden automatisch der Standardmodus des Monitors verwendet, ohne weitere Rückfrage.\n\n" +
            "Trotzdem übernehmen?",
            "Nicht unterstützter Wert", MessageBoxButton.OKCancel, MessageBoxImage.Warning, MessageBoxResult.Cancel);
